...
const reduceCart = (acc, val) => {
    const findEle = acc.find((item) => item.id === val.id);
    if (findEle === undefined) {
      return acc.concat(val);
    }
    return acc.map((item) =>
      item.id === findEle.id ? { ...item, qty: item.qty + 1 } : item
    );
  };
...
const dataReducer = (initState, dispatch)=> {
    ...
    switch(dispatch.type){
        ...
        case CART.INCREASE_QTY: {
          return {
            ...state,
            cartItems: cartItems.map(item => item.id === dispatch.payload ? {...item, qty: item.qty + 1} : item)
          }
        }
        ...
        case WISH.REMOVE_WISH_ITEM: {
          return {
            ...state,
            wishItems: wishItems.filter(item => item.id !== dispatch.payload)
          }
        }
        default:
            return {...state}
